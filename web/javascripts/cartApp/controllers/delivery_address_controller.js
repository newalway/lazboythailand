app.controller('deliveryAddressCtrl',['$scope', '$sce', 'product_data', function($scope, $sce, product_data){
	...
	$scope.submitDeliveryAddressForm = function(tmp_country_code)
	{
		...
		product_data.addCheckoutDeliveryAddress($scope.frm_delivery).then(function onSuccess(response){
			if(response.data.arr_result.status){
				...
				$scope.frm_delivery = {};
				//close modal deliveryAddressModal
				angular.element($('#deliveryAddressModal').modal('toggle'));
	...
	$scope.submitCreateFirstDeliveryAddressForm = function(tmp_country_code)
	{
		...
		$scope.frm_first_delivery.district = angular.element('#fd_district').val();
		$scope.frm_first_delivery.amphure = angular.element('#fd_amphure').val();
		$scope.frm_first_delivery.province = angular.element('#fd_province').val();
		$scope.frm_first_delivery.postcode = angular.element('#fd_postcode').val();
