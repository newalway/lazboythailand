...
class ShowroomRepository extends EntityRepository
{
    public function findAllData($arr_query_data=false)
    {
        $qb = $this->createQueryBuilder('s')
                ->orderBy('s.isHighlight', 'DESC')
                ->addOrderBy('s.positionIsHighlight', 'ASC')
        ...
        if(isset($arr_query_data['is_highlight'])){
            $qb->andWhere('s.isHighlight = '.$arr_query_data['is_highlight']);
        }
    ...
    public function findActiveDataByIsHighlight()
    {
        $qb = $this->findAllActiveData();
        $qb->andWhere('s.isHighlight = 1');
        return $qb;
    }
