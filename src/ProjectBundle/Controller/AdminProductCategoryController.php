...
// $node_id = 2;
// $product_category_node = $repo->find($node_id);
// $arrayTree = $repo->childrenHierarchy($product_category_node);
// print_r($arrayTree);
//
// $query = $repo->findChildrenByParentId($node_id)->getQuery();
// $options = $util->getInputSelectTreeViewOptions();
// $option_tree = $repo->buildTree($query->getArrayResult(), $options);
// print_r($option_tree);
//
// //include Node
// $pr_category = $repo->find($node_id);
// $children = $repo->children($pr_category, false, null, 'asc', true);
// foreach ($children as $chi) {
// 	$parent = $chi->getParent();
// 	$arr_cat_id[] = $chi->getTitle().' <<-- parent == '.$parent->getTitle();
// }
// $data['cat'] = $arr_cat_id;
// print_r($data);
// exit;

// Add root node variant
// $repo = $em->getRepository(ProductCategory::class);
// $variant = new ProductCategory();
// $variant->translate('en')->setTitle('Variant');
// $variant->translate('th')->setTitle('Variant');
// $em->persist($variant);
// $variant->mergeNewTranslations();
// $em->flush();
// <<<<<
	...
	*/
	public function updateAction(Request $request)
	{
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			//remove banner image
			if($request->get('removefilebanner_image')==1){
				$data->removeBannerImage();
			}
