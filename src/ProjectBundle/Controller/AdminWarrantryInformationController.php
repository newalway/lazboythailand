...
class AdminWarrantryInformationController extends Controller
{
    const ROUTER_INDEX = 'admin_warrantry_information';
    const ROUTER_CONTROLLER = 'AdminWarrantryInformation';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'warrantry_information']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('Warrantry Information');
            $data->translate('th')->setTitle('การรับประกัน');
            $data->setName('warrantry_information');
            $em->persist($data);
