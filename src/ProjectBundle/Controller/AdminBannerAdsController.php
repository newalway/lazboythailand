...
use ProjectBundle\Form\Type\AdminBannerAdsType;
...
class AdminBannerAdsController extends Controller
{
	const ROUTER_INDEX = 'admin_banner_ads';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  	const ROUTER_PREFIX = 'banner_ads';
	const ROUTER_CONTROLLER = 'AdminBannerAds';

	protected function getQuerySearchData($arr_query_data,$locale)
	{
		$repository = $this->getDoctrine()->getRepository(BannerAds::class);
	...
	*/
	public function newAction(Request $request)
	{
		...
		$form = $this->createForm(AdminBannerAdsType::class, new BannerAds());
	...
	*/
	public function createAction(Request $request)
	{
		...
		$data = new BannerAds();
		$form = $this->createForm(AdminBannerAdsType::class, $data);
	...
	*/
	public function editAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$util->setCkAuthorized();
		$data = $this->getDoctrine()->getRepository(BannerAds::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }

		$form = $this->createForm(AdminBannerAdsType::class, $data, array('allow_extra_fields'=>true));
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(BannerAds::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(BannerAds::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
						$em->flush();
