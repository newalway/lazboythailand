...
class MemberController extends Controller
{
	...
	*/
	public function wishlistAction(Request $request)
	{
		...
		$product_wishlist = $em->getRepository(Product::Class)->findActiveDataByUser($user);

		$paginated = $util->setPaginatedOnPagerfanta($product_wishlist,10);


		return $this->render('ProjectBundle:'.$this->container->getParameter('view_member').':wishlist.html.twig',array(
			"paginated"=>$paginated,
		));
	}
	...
	*/
	public function addWishlistAction(Request $request)
	{
		...
		// $data = $em->getRepository(Product::class)->findByUser($user)->getQuery()->getResult();
		$products = $user->getProducts();
		$data_products = array();
		if($products){
			foreach ($products as $product) {
				$data_products[] =  $product->getId();
		...
		//$chkWishlist = $em->getRepository(UsersProducts::class)->findByProducts($data[0][0])->getQuery()->getResult();
		//$chkWishlist = $data[0][0]->getUsers();

		try {
			if(in_array($request->get('id'),$data_products)){
				$user->removeProducts($data);
				$status = "remove success";
				$em->flush();
			}else{
				$user->addProducts($data);
				$status = "add success";
				$em->flush();
			}
			...
			$status = 'warning'."Can't Add ".$data->getTitle();
		}
	...
	* @Secure(roles="ROLE_CUSTOMER")
	*/
	public function wishlistDeleteAction(Request $request)
	{
		$session = $request->getSession();
		$util = $this->container->get('utilities');
		$em = $this->getDoctrine()->getManager();
		...
		return $this->redirect($util->getBackToUrl("member_wishlist"));
	}

	/**
	* @Secure(roles="ROLE_CUSTOMER")
	*/
	public function _wishlistCountAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$em = $this->getDoctrine()->getManager();
		$session = $request->getSession();
		...
		$data = $em->getRepository(Product::class)->findActiveDataByUser($user)->getQuery()->getResult();
		if($data){
			$count_text= count($data);
		}else{
			$count_text='0';
		}
