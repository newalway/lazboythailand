...
class AdminProductTypeController extends Controller
{
	...
	*/
	public function newAction(Request $request)
	{
		...
		$form = $this->createForm(AdminProductTypeType::class, $data, array('allow_extra_fields'=>true));
	...
	*/
	public function editAction(Request $request)
	{
		...
		$data = $this->getDoctrine()->getRepository(ProductType::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }

		$form = $this->createForm(AdminProductTypeType::class, $data);
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(ProductType::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
		$form = $this->createForm(AdminProductTypeType::class, $data);
		$form->handleRequest($request);
	...
	public function sort_prosessAction(Request $request)
	{
		...
		if ($sorted) {
			foreach ($sorted as $data_id) {
				$data = $em->getRepository(ProductType::class)->find($data_id);
				if ($data) {
					$i=$i+1;
					$data->setPosition($i);
				}
