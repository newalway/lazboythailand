...
use ProjectBundle\Form\Type\AdminProductStyleNumberType;

class AdminProductStyleNumberController extends Controller
{
	const ROUTER_INDEX = 'admin_product_style_number';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
	const ROUTER_CONTROLLER = 'AdminProductStyleNumber';

	protected function getQuerySearchData($arr_query_data)
	{
		$repository = $this->getDoctrine()->getRepository(ProductStyleNumber::class);
	...
	*/
	public function newAction(Request $request)
	{
		...
		$data = new ProductStyleNumber();
		$form = $this->createForm(AdminProductStyleNumberType::class, $data, array('allow_extra_fields'=>true));
	...
	*/
	public function editAction(Request $request)
	{
		...
		$data = $this->getDoctrine()->getRepository(ProductStyleNumber::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }

		$form = $this->createForm(AdminProductStyleNumberType::class, $data);
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(ProductStyleNumber::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
		$form = $this->createForm(AdminProductStyleNumberType::class, $data);
		$form->handleRequest($request);
	...
	public function sort_prosessAction(Request $request)
	{
		...
		if ($sorted) {
			foreach ($sorted as $data_id) {
				$data = $em->getRepository(ProductStyleNumber::class)->find($data_id);
				if ($data) {
					$i=$i+1;
					$data->setPosition($i);
				}
