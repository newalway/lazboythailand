...
class AdminProductInformationController extends Controller
{
    const ROUTER_INDEX = 'admin_product_information';
    const ROUTER_CONTROLLER = 'AdminProductInformation';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'product_information']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('Product Information');
            $data->translate('th')->setTitle('ข้อมูลผลิตภัณฑ์');
            $data->setName('product_information');
            $em->persist($data);
