...
use ProjectBundle\Form\Type\AdminVideosType;
...
{
	const ROUTER_INDEX = 'admin_videos';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  	const ROUTER_PREFIX = 'videos';
	const ROUTER_CONTROLLER = 'AdminVideos';
	...
	*/
	public function newAction(Request $request)
	{
		...
		$data = new Videos();
		$form = $this->createForm(AdminVideosType::class, $data, array('allow_extra_fields'=>true));
	...
	*/
	public function editAction(Request $request)
	{
		...
		$util->setCkAuthorized();
		$acctoken = $util->getAccessToken();
		$data = $this->getDoctrine()->getRepository(Videos::class)->find($request->get('id'));
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$acctoken = $util->getAccessToken();
		$em = $this->getDoctrine()->getManager();
		$data = $em->getRepository(Videos::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(Videos::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
				    $em->flush();
