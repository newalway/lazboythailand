...
class AdminIcleanWarranteeController extends Controller
{
    const ROUTER_INDEX = 'admin_iclean_warrantee';
    const ROUTER_CONTROLLER = 'AdminIcleanWarrantee';
    ...
    */
    public function indexAction(Request $request)
    {
        ...
        $data = $em->getRepository(Pages::class)->findOneBy(['name'=>'iclean_warrantee']);
        if (!$data) {
            //set default data, with translator
            $data = new Pages();
            $data->translate('en')->setTitle('iClean™ Warrantee Information');
            $data->translate('th')->setTitle('iClean™ Warrantee Information');
            $data->setName('iclean_warrantee');
            $em->persist($data);
