...
class ApiProductController extends FOSRestController
{
	...
	*/
	...
	/**
	* List products inspirations.
	*
	* @ApiDoc(
	*  resource=true,
	*  description="List products inspirations",
	*   statusCodes = {
	*     200 = "Returned when successful"
	*   }
	* )
	...
	* @return array
	*/
	public function getProductsInspirationsAction(Request $request)
	{
		$inspiration_id = $request->get('inspiration_id');

		$arr_products = array();
		$products = $this->getDoctrine()->getRepository(Product::class)->findProductsInspirationsByInspirationIdDataJoined($inspiration_id)
			->getQuery()
			->getResult();
	...
	/**
	* List products featuress.
	*
	* @ApiDoc(
	*  resource=true,
	*  description="List products featuress",
	*   statusCodes = {
	*     200 = "Returned when successful"
	*   }
	* )
	...
	* @return array
	*/
	public function getProductsFeaturessAction(Request $request)
	{
		$features_id = $request->get('features_id');

		$arr_products = array();
		$products = $this->getDoctrine()->getRepository(Product::class)->findProductsFeaturessByFeaturesIdDataJoined($features_id)
			->getQuery()
			->getResult();
	...
	*/
	public function getProductVariantsAction(Request $request)
	{
		...
		$cover_product_category_data = array();
		...
		if ($product) {
			...
			$cover_selected_data = $arr_data['arr_sku_variant_data']['cover_selected_data'];
			$cover_product_category_data = $arr_data['arr_sku_variant_data']['cover_product_category_data'];
		}

		//get cover variants
		$arr_cover_variants = $product_util->getArrayGroupVariants();
	    ...
	    $json_response->setData(array(
					...
					"arr_cover_variants" => $arr_cover_variants,
					"cover_product_category_data" => $cover_product_category_data,
					"cover_selected_data"=>$cover_selected_data
	    ));
		return $json_response;
	}

	/**
		 * Post wishlist data by user.
		 *
     * @ApiDoc(
		 *   resource = true,
     *   description = "Post wishlist data by user",
     * )
     *
	   *
	   * @param Request $request the request object
	   ...
	   */
	public function postAddProductWishlistAction(Request $request)
	{
		//$session = $request->getSession();
		// $util = $this->container->get('utilities');
	 	// $em = $this->getDoctrine()->getManager();
		//  $user = $this->getUser();
		//  if(!$user){throw $this->createAccessDeniedException('This User not found in the database');}
		//
		// $data = $em->getRepository(Product::class)->getActiveDataById($request->get('id'))->getQuery()->getResult();
		//
		// if (!$data) { throw $this->createNotFoundException('No data found'); }
		// try {
		// $user->addProducts($data[0][0]);
		// $em->flush();
		// 	$util->setRemoveNotice();
		// } catch(\Doctrine\DBAL\DBALException $e) {
		// 	$util->setCustomeFlashMessage('warning', $msg="Can't Add ".$data->getTitle());
		// }
	...
	* @return array
	*/
	public function getGroupVariantsAction(Request $request)
	{
		$em = $this->getDoctrine()->getManager();
		$product_util = $this->container->get('app.product');
