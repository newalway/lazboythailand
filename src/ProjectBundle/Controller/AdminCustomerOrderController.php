...
class AdminCustomerOrderController extends Controller
{
	...
	*/
	public function indexAction(Request $request)
	{
		...
		$query = $repository->findCustomerOrderWithoutQuotation($arr_query_data, $payment_quotation_code);
	...
	*/
	public function updateStatusAction(CustomerOrder $customer_orderObj,Request $request)
	{
		...
		$notice_message = 'Data successfully saved';
		...
		}elseif($req_status == $payment_status_delivered){
			$customer_orderObj->setDelivered(1);
			$em->flush($customer_orderObj);

		}elseif($req_status == $payment_status_cancelled){
			$customer_orderObj->setCancelled(1);
			$em->flush($customer_orderObj);
			// $this->get('session')->getFlashBag()->add('notice', 'Cancelled successful');

		}elseif($req_status == $payment_status_refunded){
			$customer_orderObj->setCancelled(1);
			$customer_orderObj->setRefunded(1);
			$em->flush($customer_orderObj);
			// $this->get('session')->getFlashBag()->add('notice', 'Refunded successful');

		}elseif($req_status=="soft_deleted"){
			$customer_orderObj->setDeleted(1);
			$em->flush($customer_orderObj);
			$this->get('session')->getFlashBag()->add('notice', 'Deleted successful');

			return $this->redirectToRoute('admin_customer_order_list');
		}else{
			$this->get('session')->getFlashBag()->add('warning', 'Error save failed');
		}

		//get order obj
		$order = $orders[0];
		$order_status_email = filter_var($util->getAuthenticationValue('order_status_email'), FILTER_VALIDATE_BOOLEAN);
		if($order_status_email){
			...
			$email_response = $util->sendOrderStatusNotificationMail($order, $order_items, $req_status);
			if($email_response['success']){
				$notice_message .= ', '.$email_response['message'];
			}else{
				$this->get('session')->getFlashBag()->add('warning', $email_response['message']);
			}
		}

		$this->get('session')->getFlashBag()->add('notice', $notice_message);
