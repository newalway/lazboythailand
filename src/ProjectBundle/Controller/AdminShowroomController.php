...
use ProjectBundle\Entity\ShowroomImage;
...
class AdminShowroomController extends Controller
{
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if(count($image_path)>0){
				foreach ($image_path as $image_uri) {
					if($image_uri){
						$gallery_image = new ShowroomImage();
						$gallery_image->setImage($image_uri);
						$gallery_image->setShowroom($data);
	...
	*/
	public function editAction(Request $request)
	{
		...
		$gallery_images = $data->getShowroomImage();
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if(is_array($del_img_gallery_ids) && count($del_img_gallery_ids)>0){
				foreach ($del_img_gallery_ids as $del_img_id) {
					$del_style_image = $em->getRepository(ShowroomImage::class)->find($del_img_id);
	...
	* @Secure(roles="ROLE_EDITOR")
	*/
	public function sort_highlightAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$arr_query_data = $util->prepare_query_data($request);
		$arr_query_data['is_highlight'] = 1;
		$datas = $this->getQuerySearchData($arr_query_data)->getQuery()->getResult();
		return $this->render('ProjectBundle:'.self::ROUTER_CONTROLLER.':sort_highlight.html.twig', array('datas' =>$datas));
	}

	/**
	* @Secure(roles="ROLE_EDITOR")
	*/
	public function sort_highlight_prosessAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$em = $this->getDoctrine()->getManager();
