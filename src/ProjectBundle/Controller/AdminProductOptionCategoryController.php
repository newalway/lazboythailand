...
use ProjectBundle\Form\Type\AdminProductOptionCategoryType;

class AdminProductOptionCategoryController extends Controller
{
	const ROUTER_INDEX = 'admin_product_option_category';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
	const ROUTER_CONTROLLER = 'AdminProductOptionCategory';

	protected function getQuerySearchData($arr_query_data)
	{
		$repository = $this->getDoctrine()->getRepository(ProductOptionCategory::class);
	...
	*/
	public function newAction(Request $request)
	{
		...
		$data = new ProductOptionCategory();
		$form = $this->createForm(AdminProductOptionCategoryType::class, $data, array('allow_extra_fields'=>true));
	...
	*/
	public function editAction(Request $request)
	{
		...
		$data = $this->getDoctrine()->getRepository(ProductOptionCategory::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }

		$form = $this->createForm(AdminProductOptionCategoryType::class, $data);
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(ProductOptionCategory::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
		$form = $this->createForm(AdminProductOptionCategoryType::class, $data);
		$form->handleRequest($request);
	...
	public function sort_prosessAction(Request $request)
	{
		...
		if ($sorted) {
			foreach ($sorted as $data_id) {
				$data = $em->getRepository(ProductOptionCategory::class)->find($data_id);
				if ($data) {
					$i=$i+1;
					$data->setPosition($i);
				}
