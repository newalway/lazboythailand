...
use ProjectBundle\Form\Type\AdminNewsType;
...
{
	const ROUTER_INDEX = 'admin_news';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  	const ROUTER_PREFIX = 'news';
	const ROUTER_CONTROLLER = 'AdminNews';
	...
	*/
	public function indexAction(Request $request)
	{
		...
		if($arr_query_data['category_id']){
			...
			$arr_query_data['category_id'] = $util->getChildrenNewsCategoryByCategoryId($searchFormData);
		}
		...
		$category_root_id = $this->container->getParameter('news_category_root_id');
	...
	*/
	public function newAction(Request $request)
	{
		...
		$author_name = $util->getNewsAuthorName($logged_in_user);
		...
		$form = $this->createForm(AdminNewsType::class, $data, array('allow_extra_fields'=>true));
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if(!empty($image_path)){
				foreach ($image_path as $image_uri) {
					if($image_uri){
						$gallery_image = new NewsImage();
						$gallery_image->setImage($image_uri);
						$gallery_image->setNews($data);
	...
	*/
	public function editAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$util->setCkAuthorized();
		$data = $this->getDoctrine()->getRepository(News::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
		$gallery_images = $data->getNewsImages();
		$form = $this->createForm(AdminNewsType::class, $data, array('allow_extra_fields'=>true));
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(News::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
		$gallery_images = $data->getNewsImages();
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if(!empty($del_img_gallery_ids)){
				foreach ($del_img_gallery_ids as $del_img_id) {
					$del_style_image = $em->getRepository(NewsImage::class)->find($del_img_id);
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(News::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
						$em->flush();
