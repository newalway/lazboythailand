...
class ExportExcel
{
	...
	public function getHeaderExcelOrder(){
		...
		$this->excelCellColor('A1:R1','1071ad');
	...
	public function setDataExcelOrder($orders)
	{
		...
		$payment_status_failed = $this->container->getParameter('payment_status_failed');
		...
		foreach ($orders as $order){
			...
			// $status = '=IF('.$order->getPaid().'=1,"paid","awaiting payment")';
			...
			}elseif($order->getFailed()){
				$status = $payment_status_failed;
			}else{
				if($order->getPaid()){
					$status = $payment_status_paid;
				}else{
					$status = $payment_status_awaiting_payment;
				}
			}
			...
			$this->spreadsheet->setActiveSheetIndex($this->index)
				  ...
				  ->setCellValue('G'.$i, ucfirst($status))
				...
				foreach ($order->getCustomerOrderDeliverys() as $deliveryAddress){
					 ...
					 $amphure = $deliveryAddress->getAmphure();
					 if($amphure){
						 $district .= ', '.$amphure;
					 }
